 ...
 * 
 */
public class MagnitudeEvaluationRule_eq extends EvaluationRule<Earthquake, Attribute> {

	@Override
	public boolean isEqual(Earthquake record1, Earthquake record2, Attribute schemaElement) {
		if(record1.getMagnitude() == -1 && record2.getMagnitude() == -1)
			return true;
		else if(record1.getMagnitude() == -1 ^ record2.getMagnitude() == -1)
			return false;
		else 
			return (int) Math.round(record1.getMagnitude()) == (int) Math.round(record2.getMagnitude());
	}
