 ...
 * 
 */
public class LongitudeFuserFavourSource_eq extends AttributeValueFuser<Double, Earthquake, Attribute> {

	public LongitudeFuserFavourSource_eq() {
		super(new FavourSources<Double, Earthquake, Attribute>());
	}

	@Override
	public boolean hasValue(Earthquake record, Correspondence<Attribute, Matchable> correspondence) {
		return record.hasValue(Earthquake.LONGITUDE);
	}
	...
	public void fuse(RecordGroup<Earthquake, Attribute> group, Earthquake fusedRecord, Processable<Correspondence<Attribute, Matchable>> schemaCorrespondences, Attribute schemaElement) {
		...
		fusedRecord.setAttributeProvenance(Earthquake.LONGITUDE, fused.getOriginalIds());
	}
